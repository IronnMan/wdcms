...
class ModuleInstaller
{
     ...
     * @param bool $dev
     */
    public function __construct($name, $version = null, $type = null, $tree = false, $dev = false)
    {
        $this->name = $name;
...
//        }
    ...
    public function getModuleName()
    {
        ...
        $name = explode("-", end($parts));

        return Str::studly($name[0]);
    }
    ...
    {
        // $remove_git = !$this->dev ? ' && rm -rf .git' : '';
        return Process::fromShellCommandline(sprintf(
            'cd %s && git clone %s %s && cd %s',
            base_path(),
            $this->getRepoUrl(),
